...
const INIT_MD5F = new DataView(Uint8Array.of(
    0x01, 0x23, 0x45, 0x67,
    0x89, 0xab, 0xdc, 0xef,
    0xfe, 0xdc, 0xba, 0x98,
    0x46, 0x57, 0x32, 0x10,
...
function rotateLeft(x: number, s: number) {
    return ((x << s) | (x >>> (32 - s))) & MASK32;
}

function updateBlock(s: Int32Array, buf: DataView) {
    let a = s[0];
    let b = s[1];
    let c = s[2];
    let d = s[3];
...
export function digest(data: ArrayBuffer) {
    const s = Int32Array.of(
        INIT_MD5F.getInt32(0 * Int32Array.BYTES_PER_ELEMENT, true),
        INIT_MD5F.getInt32(1 * Int32Array.BYTES_PER_ELEMENT, true),
