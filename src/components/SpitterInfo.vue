...
const circlePos = (i: number) => {
    ...
    if (singleLine) {
        pos1.setFromMatrixPosition(props.selectBox);
        props.cameraPosVersion;
        pos1.applyMatrix4(props.camera.matrixWorldInverse);
        const up = i % 2 === 1 ? 1 : -1;
        offset.set(pos1.x, -pos1.y).normalize().multiplyScalar(offsetR * up);
        c.add(offset);
    }
    return c;
}
